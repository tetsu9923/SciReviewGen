...
class BartEncoder(BartPretrainedModel):
    ...
    def forward(
        ...
        input_ids = torch.ones(len(input_ids_list), l, dtype=torch.int32, device="cuda")
        attention_mask = torch.zeros(len(attention_mask_list), l, dtype=torch.int32, device="cuda")
        ...
        for i, (_hidden_states, _attention_mask) in enumerate(zip(hidden_states, original_attention_mask)):
            if i == 0:
                title_hidden_states = torch.mean(_hidden_states.index_select(0, torch.tensor([i for i, mask in enumerate(_attention_mask) if mask == 1], device="cuda")), dim=0)
            else:
        ...
        for i, (_hidden_states, _attention_mask) in enumerate(zip(hidden_states, original_attention_mask)):
            if i == 0:
                return_hidden_states = _hidden_states.index_select(0, torch.tensor([i for i, mask in enumerate(_attention_mask) if mask == 1], device="cuda"))
            else:
