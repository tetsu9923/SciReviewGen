...
if __name__ == "__main__":
    save_metadata_survey_df(args)
    save_metadata_outbound_df(args)
    save_pdf_df(args)
...
def save_pdf_df(args):
    ...
    for i in tqdm(range(100)):
        pdf_df = pd.read_json(os.path.join(args.s2orc_path, 'metadata/metadata_{}.jsonl.gz'.format(i)), lines=True, compression='infer')
        pdf_df = pdf_df.set_index('paper_id')
        pdf_df.index = pdf_df.index.astype('str')
    
        pdf_survey_paper_ids = list(set(survey_index) & set(pdf_df.index))
        pdf_survey_df = pdf_df.loc[pdf_survey_paper_ids]
        if not os.path.exists(os.path.join(args.dataset_path, 'pdf_survey')):
            os.makedirs(os.path.join(args.dataset_path, 'pdf_survey'))
        pdf_survey_df.to_pickle(os.path.join(args.dataset_path, 'pdf_survey/pdf_survey_df{}.pkl'.format(i)))
    
        pdf_outbound_paper_ids = list(set(outbound_index) & set(pdf_df.index))
        pdf_outbound_df = pdf_df.loc[pdf_outbound_paper_ids]
        if not os.path.exists(os.path.join(args.dataset_path, 'pdf_outbound')):
            os.makedirs(os.path.join(args.dataset_path, 'pdf_outbound'))
        pdf_outbound_df.to_pickle(os.path.join(args.dataset_path, 'pdf_outbound/pdf_outbound_df{}.pkl'.format(i)))

    pdf_survey_dfs = []
    for i in tqdm(range(100)):
        pdf_survey_df = pd.read_pickle(os.path.join(args.dataset_path, 'pdf_survey/pdf_survey_df{}.pkl'.format(i)))
        pdf_survey_df = pdf_survey_df[['body_text', 'bib_entries']]

        pdf_survey_df["title"] = ""
        pdf_survey_df["abstract"] = ""
        bar = tqdm(total=len(pdf_survey_df))
        for i, row in enumerate(pdf_survey_df.itertuples()):
            pdf_survey_df.at[pdf_survey_df.index[i], "title"] = metadata_survey_df.query('paper_id == @row.Index')["title"].item()
            pdf_survey_df.at[pdf_survey_df.index[i], "abstract"] = metadata_survey_df.query('paper_id == @row.Index')["abstract"].item()
            bar.update(1)
    ...
    pdf_outbound_dfs = []
    for i in tqdm(range(100)):
        pdf_outbound_df = pd.read_pickle(os.path.join(args.dataset_path, 'pdf_outbound/pdf_outbound_df{}.pkl'.format(i)))
        pdf_outbound_df = pdf_outbound_df[pdf_outbound_df.body_text.apply(lambda text: len(text) > 0)]
        pdf_outbound_df = pdf_outbound_df[['body_text', 'bib_entries']]
        pdf_outbound_dfs.append(pdf_outbound_df)
    pdf_outbound_df = pd.concat(pdf_outbound_dfs)
